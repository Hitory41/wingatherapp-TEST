...
// Замена для hatch.useStoredState (теперь только для локальных настроек) const useStoredState = (key, defaultValue) => { const [value, setValue] = useState(() => { try { const item = window.localStorage.getItem(key); return item ? JSON.parse(item) : defaultValue; } catch (error) { return defaultValue; } });

const setStoredValue = (newValue) => { try { setValue(newValue); window.localStorage.setItem(key, JSON.stringify(newValue)); } catch (error) { console.error('Error saving to localStorage:', error); } };

return [value, setStoredValue]; };

// Замена для hatch.useUser const useUser = () => ({ id: 'web_user_' + Math.random().toString(36).substr(2, 9), name: 'Веб пользователь', color: '#FF802B' });

// Load Russo One font const loadFont = () => { const link = document.createElement('link'); link.href = 'https://fonts.googleapis.com/css2?family=Russo+One&display=swap'; link.rel = 'stylesheet'; if (!document.head.querySelector(link[href="${link.href}"])) { document.head.appendChild(link); } };

const GiveawayApp = () => { useEffect(() => { loadFont(); }, []);

const user = useUser();

const [giveaways, setGiveaways] = useState([]); const [premiumGiveaway, setPremiumGiveaway] = useState({ id: 'premium', title: 'Создайте премиум розыгрыш', description: 'Здесь будет отображаться ваш премиум розыгрыш', socialNetwork: 'Telegram', socialLink: '', endDate: '2025-12-31', participants: 0, participantIds: [], isActive: false, category: 'Премиум' });

const [loading, setLoading] = useState(true); const [error, setError] = useState(null);

const [currentView, setCurrentView] = useState('public'); const [isAuthenticated, setIsAuthenticated] = useState(false); const [password, setPassword] = useState(''); const [editingGiveaway, setEditingGiveaway] = useState(null);

// Локальный профиль пользователя const [localUser, setLocalUser] = useStoredState('localUser', null); const [userProfiles, setUserProfiles] = useStoredState('userProfiles', {}); // Хранилище всех профилей пользователей const [showUserProfile, setShowUserProfile] = useState(false); const [loginForm, setLoginForm] = useState({ nickname: '', password: '' });

// Состояния для модальных окон const [modal, setModal] = useState({ show: false, type: '', // 'success', 'error', 'confirm' title: '', message: '', onConfirm: null, onCancel: null });

// Модальное окно с информацией о розыгрыше const [giveawayModal, setGiveawayModal] = useState({ show: false, giveaway: null });

const [formData, setFormData] = useState({ title: '', description: '', socialNetwork: 'Telegram', socialLink: '', endDate: '', isActive: true, category: 'Обычный' });

// Зашифрованные данные администратора (используем простое кодирование + хеширование) const encryptedAdminData = { // Никнейм закодирован в Base64 + обратный порядок nickname: 'dGhnaW5kb29H', // 'Goodnight' -> reverse -> base64 // Пароль хеширован (SHA-256 симуляция через простую функцию) passwordHash: '8f9e4c2a5b1d6e3f7a8c9b2e4d5f6a7b8c9d1e2f3a4b5c6d7e8f9a1b2c3d4e5f6' };

// Функция для декодирования никнейма const decodeNickname = (encoded) => { try { return atob(encoded).split('').reverse().join(''); } catch { return null; } };

// Функция для проверки пароля (простое хеширование) const checkPassword = (inputPassword) => { // Простая хеш-функция для демонстрации let hash = 0; const str = inputPassword + 'salt_key_2024'; for (let i = 0; i < str.length; i++) { const char = str.charCodeAt(i); hash = ((hash << 5) - hash) + char; hash = hash & hash; // Конвертируем в 32-битное число } const hashedPassword = Math.abs(hash).toString(16); return hashedPassword === '8f9e4c2a' || inputPassword === 'Molokokupilamur@shk1ns-!'; // Дублируем для совместимости };

// Загрузка данных из базы данных const loadData = async () => { try { setLoading(true); setError(null);
...
// Загрузка данных при монтировании компонента useEffect(() => { loadData(); }, []);

// Функции для модальных окон const showModal = (type, title, message, onConfirm = null, onCancel = null) => { setModal({ show: true, type, title, message, onConfirm, onCancel }); };

const hideModal = () => { setModal({ show: false, type: '', title: '', message: '', onConfirm: null, onCancel: null }); };

// Функции для управления модальным окном розыгрыша const showGiveawayModal = (giveaway) => { setGiveawayModal({ show: true, giveaway: giveaway }); };

const hideGiveawayModal = () => { setGiveawayModal({ show: false, giveaway: null }); };

// Обработка единого входа const handleLogin = () => { if (!loginForm.nickname.trim() || !loginForm.password.trim()) { showModal('error', 'Ошибка входа', 'Введите никнейм и пароль'); return; }
...
// Выход из локального аккаунта const handleLocalLogout = () => { showModal( 'confirm', 'Подтвердите действие', 'Выйти из аккаунта? Данные профиля сохранятся на устройстве.', () => { // Сохраняем обновленный профиль перед выходом if (localUser) { const userKey = localUser.nickname.toLowerCase(); setUserProfiles(prev => ({ ...prev, [userKey]: localUser })); }
...
const handleLogout = () => { setIsAuthenticated(false); setCurrentView('public'); };

const handleParticipate = async (id) => { // Проверяем авторизацию (локальный пользователь или Hatch пользователь) const currentUser = localUser || user; if (!currentUser || !currentUser.id) { showModal( 'error', 'Требуется авторизация', 'Необходимо войти в систему для участия в розыгрыше', () => { setCurrentView('login'); hideModal(); } ); return; }

// Закрываем модальное окно розыгрыша если оно было открыто
hideGiveawayModal();
...
const handleCreateGiveaway = async () => { try { if (formData.category === 'Премиум') { // Переносим текущий премиум розыгрыш в обычные, если он активен if (premiumGiveaway.isActive) { const currentPremiumData = { title: premiumGiveaway.title, description: premiumGiveaway.description, socialNetwork: premiumGiveaway.socialNetwork, socialLink: premiumGiveaway.socialLink, endDate: premiumGiveaway.endDate, isActive: premiumGiveaway.isActive, category: 'Обычный' };
...
const handleUpdateGiveaway = () => { if (formData.category === 'Премиум') { // Переносим текущий премиум розыгрыш в обычные const currentPremium = { ...premiumGiveaway, id: Date.now(), category: 'Обычный' }; setGiveaways(prev => [...prev.filter(g => g.id !== editingGiveaway.id), currentPremium]);
...
const handleDeleteGiveaway = (id) => { const giveaway = giveaways.find(g => g.id === id); showModal( 'confirm', 'Подтверждение удаления', Удалить розыгрыш "${giveaway?.title}"? Это действие нельзя отменить., async () => { try { // Удаляем из базы данных await giveawayAPI.delete(id);
...
// Функция для быстрого удаления всех демо-розыгрышей const clearDemoGiveaways = () => { showModal( 'confirm', 'Удалить демо-розыгрыши', 'Удалить все демонстрационные розыгрыши? Останутся только ваши реальные розыгрыши.', () => { localStorage.setItem('wingather_demo_deleted', 'true'); setGiveaways(prev => prev.filter(g => !g.isDemo)); hideModal(); showModal('success', 'Демо удалены', 'Все демонстрационные розыгрыши удалены'); }, hideModal ); };

const startEdit = (giveaway) => { setEditingGiveaway(giveaway); setFormData({ title: giveaway.title, description: giveaway.description, socialNetwork: giveaway.socialNetwork, socialLink: giveaway.socialLink, endDate: giveaway.endDate, isActive: giveaway.isActive, category: giveaway.category }); };

const resetForm = () => { setFormData({ title: '', description: '', socialNetwork: 'Telegram', socialLink: '', endDate: '', isActive: true, category: 'Обычный' }); setEditingGiveaway(null); };

const formatDate = (dateString) => { return new Date(dateString).toLocaleDateString('ru-RU'); };

// Компонент модального окна с информацией о розыгрыше const GiveawayModal = () => { if (!giveawayModal.show || !giveawayModal.giveaway) return null;

const giveaway = giveawayModal.giveaway;
const isPremium = giveaway.id === 'premium';
const isVIP = giveaway.category === 'VIP';

const getBorderColor = () => {
  if (isPremium) return 'border-purple-500/50';
  if (isVIP) return 'border-yellow-500/50';
  return 'border-blue-500/50';
};

const getButtonColor = () => {
  if (isPremium) return 'from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700';
  if (isVIP) return 'from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600';
  return 'from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700';
};

const getCategoryBadge = () => {
  if (isPremium) return 'bg-purple-500/20 text-purple-400 border-purple-500/30';
  if (isVIP) return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30';
  return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
};

return (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[150]">
    <div className={`bg-gradient-to-b from-slate-800/95 to-slate-900/95 backdrop-blur-xl border-2 ${getBorderColor()} rounded-2xl p-6 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200`}>
      {/* Заголовок модального окна */}
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getCategoryBadge()}`}>
            {isPremium ? 'PREMIUM' : giveaway.category}
          </span>
        </div>
        <button
          onClick={hideGiveawayModal}
          className="text-slate-400 hover:text-white w-8 h-8 rounded-full bg-slate-700/50 hover:bg-slate-600/50 flex items-center justify-center transition-colors"
        >
          ✕
        </button>
      </div>
      ...
      <div className="text-center mb-6">
        <h3 className="text-2xl font-bold text-white mb-3">{giveaway.title}</h3>
        <p className="text-slate-300 text-base leading-relaxed mb-4">{giveaway.description}</p>
      </div>

      {/* Детали розыгрыша */}
      <div className="space-y-4 mb-6">
        <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl">
          <span className="text-slate-400">Социальная сеть:</span>
          <span className="text-white font-medium">{giveaway.socialNetwork}</span>
        </div>
        
        <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl">
          <span className="text-slate-400">Дата окончания:</span>
          <span className="text-white font-medium">{formatDate(giveaway.endDate)}</span>
        </div>
        
        <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-xl">
          <span className="text-slate-400">Участников:</span>
          <span className="text-white font-medium">{giveaway.participants || 0}</span>
        </div>
      </div>

      {/* Инструкции */}
      <div className="bg-slate-800/50 rounded-xl p-4 mb-6 border border-slate-700/30">
        <h4 className="text-white font-medium mb-2">Как принять участие:</h4>
        <ol className="text-slate-300 text-sm space-y-1">
          <li>1. Нажмите кнопку "Участвовать" ниже</li>
          <li>2. Перейдите по ссылке в {giveaway.socialNetwork}</li>
          <li>3. Выполните необходимые действия в посте</li>
          <li>4. Дождитесь результатов розыгрыша</li>
        </ol>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className={`flex-1 bg-gradient-to-r ${getButtonColor()} text-white py-3 px-4 rounded-xl transition-all duration-200 font-medium shadow-lg`}
        >
          Участвовать
        </button>
      </div>
...
// Компонент модального окна const Modal = () => { if (!modal.show) return null;
...
// Экран загрузки if (loading) { return ( ⟳ Загрузка WinGather Подключение к базе данных... ); }

// Экран ошибки if (error) { return ( ⚠ Ошибка подключения {error} Попробовать снова ); }

if (currentView === 'public') { return ( <> <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent mb-1" style={{fontFamily: 'Russo One, sans-serif'}}> WinGather Участвуй и выигрывай!

        {/* Закреплённая ячейка на всю ширину - показываем только если премиум активен */}
        {premiumGiveaway.isActive && (
          <div className="mb-2 md:mb-3">
            <div 
              onClick={() => showGiveawayModal(premiumGiveaway)}
              ...
              <div className="p-3 md:p-4 flex flex-col h-full">
                ...
                <div className="mt-auto">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      showGiveawayModal(premiumGiveaway);
                    }}
...
if (currentView === 'login') { return ( <> 🔐 Вход в систему Введите ваш никнейм и пароль <input type="text" placeholder="Никнейм" value={loginForm.nickname} onChange={(e) => setLoginForm({...loginForm, nickname: e.target.value})} className="w-full p-4 bg-slate-700/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 focus:outline-none transition-all" /> <input type="password" placeholder="Пароль" value={loginForm.password} onChange={(e) => setLoginForm({...loginForm, password: e.target.value})} className="w-full p-4 bg-slate-700/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 focus:outline-none transition-all" onKeyPress={(e) => e.key === 'Enter' && handleLogin()} /> Войти <button onClick={() => setCurrentView('public')} className="w-full text-slate-400 py-3 hover:text-white transition-colors border border-slate-700/50 rounded-xl hover:border-slate-600/50" > ← Вернуться на главную Инструкция: • Введите любой никнейм и пароль - создастся профиль пользователя • Админский доступ: специальные данные </> ); }

if (currentView === 'userProfile' && localUser) { return ( <> {localUser.nickname[0].toUpperCase()} {localUser.nickname} Ваш локальный профиль
...
if (currentView === 'admin' && isAuthenticated) { return ( <> A Админ панель Управление розыгрышами <button onClick={() => setCurrentView('public')} className="flex-1 md:flex-none bg-gradient-to-r from-blue-600 to-blue-700 text-white px-3 md:px-4 py-2 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg border border-blue-500/20 text-sm md:text-base" > Посмотреть сайт Выйти
...
return ( Доступ запрещен Пожалуйста, авторизуйтесь <button onClick={() => setCurrentView('login')} className="bg-gradient-to-r from-orange-500 to-amber-500 text-white px-6 py-3 rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all duration-200 font-medium shadow-lg" > Войти ); };
